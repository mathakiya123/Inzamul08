...
<h5 class="fw-bold mb-0">Manage All Users</h5>
...
<h6 class="fw-bold mb-3">Manage all users </h6>
@if(Session('success'))
...
<tr>
<td>{{$row->id}}</td>
<td><img src="{{asset('/uploads/customers/'.$row->photo)}}" alt="photo" class="w-25" /></td>
<td>{{$row->fullname}}</td>
<td>{{$row->email}}</td>
<td>{{$row->phone}}</td>
<td><a href='{{asset("/admin-login/manage-users/".$row->id)}}'><button class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure to delete users ?')"><span class="bi bi-trash"></button></a>
</td>
</tr>
@endforeach
</tbody>
